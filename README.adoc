...
Nejdokonalejší typografické integrace obrázků a diagramů do BP dosáhnete pokud je vytvoříte rovnou pomocí (La)TeXu.
...
Pokud chcete překládat jenom některou z v takto vložených kapitol (např. kvůli časové náročnosti překladu kapitoly na které už nepracujete, nebo chcete školiteli poslat jenom již hotovou část) a chcete zachovat správné číslování, https://en.wikibooks.org/wiki/LaTeX/Modular_Documents#Using_.5Cincludeonly[použijte] `\include` a `\includeonly`.
